...
> **Status: in development (M0).** Not yet usable. See the milestone table below.
...
packages/i18n      ICU message catalogs, shared by all three.
...
[dropping the OAuth proxy](docs/adr/0003-drop-the-oauth-proxy.md).
...
| M1     | LeanCloud migration tool (export → transform → load → verify)            | Next   |
...
| M5     | Auth: sessions, argon2id, TOTP, first-party OAuth                        |        |
| M6     | Admin dashboard (Vue 3)                                                  |        |
| M7     | Embed widget (Vue 3)                                                     |        |
| M8–M10 | Staging, cutover, decommission                                           |        |

M1 comes before any feature work on purpose: it is the deadline-bound piece, and
building it first forces the schema to be real before anything depends on it.
